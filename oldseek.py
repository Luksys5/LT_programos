					...
					if zinterval == [''] and zchint != ['']:
						...
						if int(chint[1]) >= int(interval[0]) and int(interval[1]) >= int(zchint[0]):
							...
							while chint[1] < interval[0]:
								...
								chint = cbind[a].split('-')
								zchint = cbind[a+1].split('-')
							print chint,zchint, interval
							diff, notf, x, y = lyginu(cbind[a],cbind[a+1], binds[b], binds[b+1])
							differs += diff
						...
						if int(interval[1]) >= int(chint[0]) and int(chint[1]) >= int(zinterval[0]):
							...
							while interval[1] < chint[0]:
								differs += binds[b]+' '
								notf += binds[b]+' '
								if zchint == ['']:
									differs += cbind[a]
									return differs, notf
							diff, notf, x, y = lyginu(chint, [''], interval, zinterval)
							differs += diff
			return differs, notf 
